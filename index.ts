 ...
 */
function listarConsultasPorPeriodo(
  consultas: Consulta[],
  dataInicio: Date,
  dataFim: Date
): Consulta[] {
  return consultas.filter(
    (consulta) => consulta.data >= dataInicio && consulta.data <= dataFim
  );
}
 ...
 */
function gerarRelatorioFaturamento(consultas: Consulta[]): string {
...
const consulta1 = criarConsulta(
  ...
  amanha,
  350,
  "Paciente com historico de hipertensao"
);
...
if (consultaCancelada) consultas.push(consultaCancelada);
